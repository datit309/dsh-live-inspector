window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    // Component to render Diff snippet clearly
    function DiffViewer({ diffs }) {
      ...
      return h('div', {
        style: {
          ...
          background: 'var(--dsw-alias-bg-layer-2, rgba(0,0,0,0.22))',
          overflow: 'hidden',
          fontSize: '11px',
        ...
        diffs.map((d, dIdx) => {
          ...
          return h('div', {
            key: dIdx,
            style: {
              borderBottom: dIdx < diffs.length - 1 ? '1px dashed var(--dsw-alias-border-l1, rgba(255,255,255,0.08))' : 'none',
              padding: '6px 8px'
            }
          },
            // Diff Header
            h('div', {
              ...
              h('span', null, d.isNewFile ? '✨ Created content' : 'Edit #' + (dIdx + 1)),
              h('span', null, '-' + oldLines.length + ' / +' + newLines.length + ' lines')
            ),

            // Deleted lines (Old string)
            oldLines.length > 0 ? h('div', {
              style: {
                background: 'rgba(239, 68, 68, 0.12)',
                borderLeft: '3px solid #ef4444',
                padding: '3px 6px',
                marginBottom: newLines.length > 0 ? '4px' : '0',
                borderRadius: '0 3px 3px 0'
              }
            },
              oldLines.slice(0, 30).map((l, lIdx) =>
                h('div', {
                  key: lIdx,
                  style: {
                    color: '#f87171',
              ...
              oldLines.length > 30 ? h('div', { style: { color: '#ef4444', fontStyle: 'italic', fontSize: '10px' } }, '... and ' + (oldLines.length - 30) + ' more lines') : null
            ) : null,

            // Added lines (New string)
            newLines.length > 0 ? h('div', {
              style: {
              ...
              newLines.slice(0, 30).map((l, lIdx) =>
                h('div', {
                  key: lIdx,
                  style: {
                    color: '#34d399',
              ...
              newLines.length > 30 ? h('div', { style: { color: '#10b981', fontStyle: 'italic', fontSize: '10px' } }, '... and ' + (newLines.length - 30) + ' more lines') : null
            ) : null
          );
        })
      );
